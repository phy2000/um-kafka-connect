...
public class UMSourceTaskTest extends EasyMockSupport {
    ...
    public void testNormalLifecycle() throws InterruptedException, IOException {
        ...
        assertEquals(Collections.singletonMap(UMSourceTask.FILENAME_FIELD, tempFile.getAbsolutePath()), records.get(0).sourcePartition());
        assertEquals(Collections.singletonMap(UMSourceTask.POSITION_FIELD, 22L), records.get(0).sourceOffset());
        assertEquals(null, task.poll());
        ...
        assertEquals(Collections.singletonMap(UMSourceTask.FILENAME_FIELD, tempFile.getAbsolutePath()), records.get(0).sourcePartition());
        assertEquals(Collections.singletonMap(UMSourceTask.POSITION_FIELD, 28L), records.get(0).sourceOffset());
        assertEquals("line2", records.get(1).value());
        assertEquals(Collections.singletonMap(UMSourceTask.FILENAME_FIELD, tempFile.getAbsolutePath()), records.get(1).sourcePartition());
        assertEquals(Collections.singletonMap(UMSourceTask.POSITION_FIELD, 35L), records.get(1).sourceOffset());
        assertEquals("line3", records.get(2).value());
        assertEquals(Collections.singletonMap(UMSourceTask.FILENAME_FIELD, tempFile.getAbsolutePath()), records.get(2).sourcePartition());
        assertEquals(Collections.singletonMap(UMSourceTask.POSITION_FIELD, 41L), records.get(2).sourceOffset());
        assertEquals("line4", records.get(3).value());
        assertEquals(Collections.singletonMap(UMSourceTask.FILENAME_FIELD, tempFile.getAbsolutePath()), records.get(3).sourcePartition());
        assertEquals(Collections.singletonMap(UMSourceTask.POSITION_FIELD, 47L), records.get(3).sourceOffset());
        ...
        assertEquals(Collections.singletonMap(UMSourceTask.FILENAME_FIELD, tempFile.getAbsolutePath()), records.get(0).sourcePartition());
        assertEquals(Collections.singletonMap(UMSourceTask.POSITION_FIELD, 48L), records.get(0).sourceOffset());
    ...
    public void testBatchSize() throws IOException, InterruptedException {
        ...
        config.put(UMSourceConnector.TASK_BATCH_SIZE_CONFIG, "5000");
        task.start(config);
